  ...
  (let* (imp
    ...
    (cond ((and
          ...
          ((or (ql-dist:find-system imp)
           ...
           (format *error-output* "System '~A' found.~%Attempting to install the scripts in roswell/ subdirectory of the system...~%" imp)
           ...
           (when *build-hook*
             (funcall *build-hook*))
           (dolist (from (directory (merge-pathnames "roswell/*.*" (ql:where-is-system imp))))
             (install-script from)))
