...
(defpackage :roswell.help.install
  (:use :cl :roswell.util :roswell.install :ros.script.help.3668211011))
(in-package :roswell.help.install)

(defun install (argv)
  (if (not (second argv))
      (let ((s *error-output*))
        (format s "Usage: ~A install impl [OPTIONS]~2%" (opt "wargv0"))
        (format s "For more details on impl specific options, type:~%")
        (format s " ~A help install impl~2%" (opt "wargv0"))
